...
## 1 背景简介
- 个人工作需求和业余知识博主爱好，需要追踪大模型的最新研究动态，及时了解最新的研究方向和算法方案，形成笔记分享给读者。
- 原先的流程是去一些论坛如智源社区，打开论文板块根据摘要来初筛感兴趣的内容，再点开论文看一些机构权威性，筛选的过程太过耗时。
- 为了提高效率，开发了这个工具，支持根据发表日期、和发表机构来批量下载arXiv上的论文，并自动分类和解读。
...
### 3.4 筛选关注机构论文
代码在 ```tools/filter_institution.py``` ，根据配置的机构清单 ```config/institution.py``` ，筛选出在关注机构清单中的论文。
