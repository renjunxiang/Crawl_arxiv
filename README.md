...
![](https://img.shields.io/badge/Python-3.10-blue.svg) ![](https://img.shields.io/badge/arxiv-2.4.0-green.svg) ![](https://img.shields.io/badge/PyPDF2-3.0.1-brightgreen.svg) ![](https://img.shields.io/badge/pdf2image-1.17.0-brightgreen.svg) ![](https://img.shields.io/badge/openai-2.0.1-brightgreen.svg)

</div>

## 更新日志
- **2026-03-31**：
  - bug修复更新：一些逻辑上的小bug改掉了；
  - 爬取限制：arxiv近期对大批量爬取，会限制访问（429错误）并禁止下载论文，特别是VPN很容易被封，增加延时机制，避免被封禁；
- **2026-03-01**：
  - 模型更新：qwen3.5-plus的性价比很高，针对性修改了每个模块的提示词；
  - 新增文案模块：为了更方便找出感兴趣的论文，新增了`prepare_materials.py`，保存论文首页图片并从笔记中提取贡献；
- **2026-02-18**：
  ...
  - ~~样例见[Crawl_arxiv/output/2026-02-15](https://github.com/liuyueyi/Crawl_arxiv/tree/main/output/2026-02-15)~~
- **2026-02-16**：
...
#### 便捷查看文案
<div align="center">
  <img src="./picture/文案样例.png" alt="文案样例" style="max-width: 600px">
</div>
...
- 提示词样例：
   ...
   ```
- 输出在 ```output/你的日期/papers_filter.json``` ，新增一个 ```category``` 字段。
...
### 3.6 生成文案（2026.3.1更新）
代码在```tools/prepare_materials.py```，汇总论文首页图片和笔记中的贡献。
- 输出在```output/你的日期/institution```，包含每一篇论文的首页图片、贡献汇总（content.txt）
